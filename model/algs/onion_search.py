...
from model.utils.algo_utils import combinations
...
class Onion_search:
    ...
    def __init__(self, world, distance_matrix, alg_args):
        ...
        self.length = int(alg_args['l']) if 'l' in alg_args else 2
    ...
    def next_step(self, current_vertex_ind, vertexes, current_time):
        while True:
            ...
            permutations = self.cluster_combinations(clusters, self.length, current_vertex_ind)
            min_price = float("inf")
    ...
    def cluster_combinations(self, clusters, length, current_vertex_ind):
        index_combos = combinations(len(clusters), length)
        combos = []
        for ind_com in index_combos:
            temp = []
            check = True
            for i in ind_com:
                temp.append(clusters[i])
                if len(temp) == 1:
                    if current_vertex_ind in temp[0]:
                        check = False
                        break
                elif len(temp) >= 2 and temp[-1] == temp[-2]:
                    check = False
                    break
            if check:
                combos.append(temp)
