...
class State:
    ...
    def branch(self):
        for i, v in enumerate(self.vertexes):
            ...
            #
            # child.price = sum(
            # [global_utils.price(v.cst(c_time_stamp), v.p) for v in child.vertexes])/(child.time_stamp-self.time_stamp) + self.price
            child.price = sum([global_utils.price(v.cst(c_time_stamp), v.p) for v in child.vertexes]) - sum(
                [global_utils.price(v.cst(c_time_stamp), v.p) for v in self.vertexes])/(child.time_stamp-self.time_stamp) + self.price
...
class K_step:
    ...
    def next_step(self, current_vertex_ind, vertexes, current_time):
        ...
        if self.is_first:
            self.is_first = False
            self.root = State(0, current_vertex_ind,
                              vertexes, self.distance_matrix)
            self.root.price = 0
            self.current_layer = []
            self.current_layer.append(self.root)
